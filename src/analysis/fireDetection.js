...
class FireDetectionAnalysis {
  constructor() {
    ...
    this.type = 'Pompier';
    this.endpoint = 'https://router.huggingface.co/hf-inference/models/EdBianchi/vit-fire-detection';
  }
  ...
  async analyze(imageBuffer) {
    try {
      ...
      if (!config.huggingface.apiKey || config.huggingface.apiKey === 'HUGGINGFACE_API_KEY') {
        ...
        return { label: 'error', score: 0, detected: false, error: errorMsg };
      }
      ...
      if (Array.isArray(response.data) && response.data.length > 0) {
        ...
        const isFireOrSmoke = topResult.label.toLowerCase().includes('fire') || 
